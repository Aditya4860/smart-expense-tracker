import { Link } from 'react-router-dom';
import { ZenithLogo } from '../pages/Landing';
import heroBg from '../assets/hero-bg-2.jpg';

/**
 * AuthLayout — premium split-panel layout shared by Login and Register.
 * Left: brand image panel | Right: form panel with Google SSO
 *
 * Props:
 *   title       — main heading text
 *   subtitle    — secondary description text
 *   footer      — JSX rendered below the card
 *   googleLabel — label for Google button (e.g. "Sign in with Google")
 *   children    — the form fields
 */
export default function AuthLayout({ title, subtitle, footer, googleLabel, children }) {
  function handleGoogleAuth() {
    const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:8000/api/v1';
    window.location.href = `${apiUrl}/oauth/google/login?t=${new Date().getTime()}`;
  }

  return (
    <div className="min-h-screen bg-black text-white flex overflow-hidden">

      {/* ── LEFT BRAND PANEL ──────────────────────────────────── */}
      <div className="hidden lg:flex flex-col justify-between w-[46%] relative p-14 overflow-hidden shrink-0">
        {/* Background image — full cover, no zoom */}
        <div className="absolute inset-0">
          <img
            src={heroBg}
            alt=""
            aria-hidden="true"
            className="w-full h-full object-cover object-center opacity-35 grayscale"
          />
          <div className="absolute inset-0 bg-gradient-to-br from-black/70 via-black/40 to-black/65" />
        </div>

        {/* Logo */}
        <Link to="/" aria-label="Back to home" className="relative z-10 flex items-center gap-2.5 group w-fit">
          <ZenithLogo size={28} />
          <span className="font-serif text-lg font-semibold tracking-wide uppercase group-hover:text-slate-200 transition-colors">Zenith</span>
        </Link>

        {/* Tagline */}
        <div className="relative z-10">
          <h2 className="font-serif text-5xl leading-[1.05] mb-6">
            Financial<br />
            <em className="italic text-slate-300">Intelligence,</em><br />
            Simplified.
          </h2>
          <p className="text-slate-400 text-base max-w-xs leading-relaxed mb-10">
            Join thousands tracking every rupee, meeting budgets, and growing wealth — in one elegant dashboard.
          </p>

          {/* Social proof */}
          <div className="flex items-center gap-3">
            <div className="flex -space-x-2">
              {['💼', '📊', '💡', '🎯'].map((e, i) => (
                <div key={i} className="w-9 h-9 rounded-full bg-white/10 border border-white/15 flex items-center justify-center text-base">
                  {e}
                </div>
              ))}
            </div>
            <p className="text-sm text-slate-400">
              <span className="text-white font-semibold">12,000+</span> users trust Zenith
            </p>
          </div>
        </div>
      </div>

      {/* ── RIGHT FORM PANEL ──────────────────────────────────── */}
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-16 bg-[#09090d] overflow-y-auto">

        {/* Mobile logo */}
        <Link to="/" className="lg:hidden flex items-center gap-2 mb-10 group">
          <ZenithLogo size={26} />
          <span className="font-serif text-lg font-semibold uppercase tracking-wide">Zenith</span>
        </Link>

        {/* Ambient glow */}
        <div aria-hidden="true" className="pointer-events-none fixed inset-0 overflow-hidden">
          <div className="absolute top-1/3 left-1/2 -translate-x-1/2 h-[350px] w-[350px] rounded-full bg-white/4 blur-[100px]" />
        </div>

        <div className="relative z-10 w-full max-w-[380px] animate-fade-up">

          {/* Header */}
          <div className="mb-8">
            <h1 className="text-2xl font-bold tracking-tight mb-2">{title}</h1>
            {subtitle && <p className="text-sm text-slate-400 leading-relaxed">{subtitle}</p>}
          </div>

          {/* Google sign-in */}
          <button
            type="button"
            id="auth-google-btn"
            onClick={handleGoogleAuth}
            className="w-full flex items-center justify-center gap-3 py-3.5 px-4 rounded-xl border border-white/12 bg-white/4 hover:bg-white/8 transition-all duration-200 hover:border-white/20 mb-6 text-sm font-medium group"
          >
            {/* Google Icon */}
            <svg className="w-5 h-5 shrink-0" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
              <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
              <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
              <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
            </svg>
            {googleLabel || 'Continue with Google'}
          </button>

          {/* Divider */}
          <div className="flex items-center gap-3 mb-6">
            <div className="flex-1 h-px bg-white/8" />
            <span className="text-xs text-slate-500">or continue with email</span>
            <div className="flex-1 h-px bg-white/8" />
          </div>

          {/* Form card */}
          <div className="rounded-2xl border border-white/8 bg-white/3 p-7 shadow-2xl">
            {children}
          </div>

          {/* Footer link */}
          {footer && (
            <div className="mt-6 text-center text-sm text-slate-500">
              {footer}
            </div>
          )}

          {/* Back home */}
          <div className="mt-8 text-center">
            <Link to="/" className="text-xs text-slate-600 hover:text-slate-400 transition-colors">
              ← Back to Zenith home
            </Link>
          </div>
        </div>
      </div>
    </div>
  );
}
