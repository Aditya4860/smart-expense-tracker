...
const ACTIONS = [
  {
    ...
    isButton:    true,   // handled by onAddExpense prop
    icon: (
  ...
  },
  {
    ...
    label:       'Analytics',
    description: 'See spending breakdown',
 ...
 */
const QuickActions = memo(function QuickActions({ onAddExpense }) {
  return (
    <section aria-label="Quick actions">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        {ACTIONS.map(action => {
          const inner = (
            <>
              <div className={['flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl', action.iconBg, action.iconText, 'transition-transform duration-200 group-hover:scale-110'].join(' ')}>
                {action.icon}
              </div>
